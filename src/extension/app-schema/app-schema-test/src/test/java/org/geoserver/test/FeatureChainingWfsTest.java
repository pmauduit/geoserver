...
public class FeatureChainingWfsTest extends AbstractAppSchemaTestSupport {
    ...
    public void testDescribeFeatureTypeMappedFeature() {
        ...
        assertXpathEvaluatesTo(
                AbstractAppSchemaMockData.GSML_SCHEMA_LOCATION_URL, "//xsd:include/@schemaLocation", doc);
        // nothing else
    ...
    /**
     * Test DescribeFeatureType response for ex:FirstParentFeature and ex:SecondParentFeature, which have two schemas in
     * the same namespace.
     */
    @Test
    ...
    /**
     * Test DescribeFeatureType response for om:Observation, which has 2 schemaUris specified in the mapping file. Both
     * must appear.
     */
    @Test
    ...
    public void testDescribeFeatureTypeNoTypes() {
        ...
        if (targetNamespace.equals(FeatureChainingMockData.EX_URI)) {
            ...
            Set<String> expectedExSchemaLocations =
                    Set.of(getExSchemaOneLocation(), getExSchemaTwoLocation(), getExSchemaThreeLocation());
     ...
     *
     * <p><strong>Remarks:</strong> this test only targets WFS 2.0, as 1.1.0 is sufficiently covered by other tests.
     */
    @Test
    public void testDescribeFeatureTypeNoWfsSchemaImport() {
        // one feature type --> schema is included, no imports
        Document doc =
                getAsDOM("wfs?service=WFS&version=2.0.0&request=DescribeFeatureType&typenames=gsml:GeologicUnit");
        assertXpathCount(0, "//xsd:import", doc);
        assertXpathCount(1, "//xsd:include", doc);
    ...
    public void testGetFeatureJSON() throws Exception {
        JSON json = getAsJSON("wfs?request=GetFeature&version=1.1"
    ...
    /**
     * GeologicUnit mapping has mappingName specified, to override targetElementName when feature chained to
     * MappedFeature. This is to test that querying GeologicUnit as top level feature still works, when its real type
     * name is specified in the query.
     */
    @Test
    ...
    /**
     * Test nesting features of complex types with simple content. Previously the nested features attributes weren't
     * encoded, so this is to ensure that this works. This also tests that a feature type can have multiple FEATURE_LINK
     * to be referred by different types.
     */
    @Test
    public void testComplexTypeWithSimpleContentGML() {
        ...
        assertXpathEvaluatesTo(
                ...
                doc);
        assertXpathCount(
                0, "//ex:FirstParentFeature[@gml:id='cc.1']/ex:nestedFeature[2]/ex:SimpleContent/FEATURE_LINK", doc);
        // cc.2
    ...
    private void checkSchemaLocation(Document doc) {
        ...
        String gsmlLocation =
                AbstractAppSchemaMockData.GSML_URI + " " + AbstractAppSchemaMockData.GSML_SCHEMA_LOCATION_URL;
    ...
    private void checkMf1Content(String id, Document doc) {
        ...
        assertXpathCount(
                0, "//gsml:MappedFeature[@gml:id='" + id + "']/gsml:specification/gsml:GeologicUnit/FEATURE_LINK", doc);
        // occurrence
        assertXpathCount(
                1,
                "//gsml:MappedFeature[@gml:id='" + id + "']/gsml:specification" + "/gsml:GeologicUnit/gsml:occurrence",
                doc);
        assertXpathEvaluatesTo(
                "",
                "//gsml:MappedFeature[@gml:id='"
        ...
        assertXpathCount(
                1,
                "//gsml:MappedFeature[@gml:id='" + id + "']/gsml:specification" + "/gsml:GeologicUnit/gsml:composition",
                doc);
        assertXpathEvaluatesTo(
                "nonexistent",
                "//gsml:MappedFeature[@gml:id='"
    ...
    /** Test anyType as complex attributes, and placeholder type (e.g AnyOrReference) which contains <any/> element. */
    @Test
    public void testAnyTypeAndAnyElementGML() {
        ...
        // om:metadata
        assertXpathEvaluatesTo(
                "651.0", "(//om:Observation)[1]/om:metadata/gsml:CGI_NumericValue/gsml:principalValue", doc);
        // om:resultQuality
        ...
        // om:metadata
        assertXpathEvaluatesTo(
                "269.0", "(//om:Observation)[2]/om:metadata/gsml:CGI_NumericValue/gsml:principalValue", doc);
        // om:resultQuality
        ...
        // om:metadata
        assertXpathEvaluatesTo(
                "123.0", "(//om:Observation)[3]/om:metadata/gsml:CGI_NumericValue/gsml:principalValue", doc);
        // om:resultQuality
        ...
        // om:metadata
        assertXpathEvaluatesTo(
                "456.0", "(//om:Observation)[4]/om:metadata/gsml:CGI_NumericValue/gsml:principalValue", doc);
        // om:resultQuality
    ...
    /** Similar to above test case but using AND as a wrapper for 2 filters involving nested attributes. */
    @Test
    ...
    public void testNestedFilterEncoding() throws FilterToSQLException, IOException {
         ...
         */
        And and = ff.and(
                ff.equals(ff.property("gsml:specification/gsml:GeologicUnit/gml:name"), ff.literal("New Group")),
                ff.equals(
                        ff.property(
    ...
    public void testNestedFilterEncodingDisabled() throws IOException, FilterToSQLException {
        ...
        AppSchemaDataAccessRegistry.getAppSchemaProperties()
                .setProperty(AppSchemaDataAccessConfigurator.PROPERTY_ENCODE_NESTED_FILTERS, "false");
        try {
            ...
            // reset default
            AppSchemaDataAccessRegistry.getAppSchemaProperties()
                    .setProperty(AppSchemaDataAccessConfigurator.PROPERTY_ENCODE_NESTED_FILTERS, "true");
        }
    }

    /**
     * Test the encoding of nested complex features that are mapped to GML complex types that don't respect the GML
    ...
    public void testNonValidNestedGML() throws Exception {
        ...
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.1']/ex:nestedFeature/ex:nestedValue[text()='1GRAV']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.1']/ex:nestedFeature/ex:nestedValue[text()='1TILL']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.1']/ex:nestedFeature/ex:nestedValue[text()='6ALLU']", result);
        ...
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.2']/ex:nestedFeature/ex:nestedValue[text()='1GRAV']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.2']/ex:nestedFeature/ex:nestedValue[text()='1TILL']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.2']/ex:nestedFeature/ex:nestedValue[text()='6ALLU']", result);
        ...
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.3']/ex:nestedFeature/ex:nestedValue[text()='1GRAV']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.3']/ex:nestedFeature/ex:nestedValue[text()='1TILL']", result);
        assertXpathCount(
                1, "//ex:ParentFeature[@gml:id='sc.3']/ex:nestedFeature/ex:nestedValue[text()='6ALLU']", result);
    }
