...
public class GetCapabilitiesScaleHintTest extends GeoServerSystemTestSupport {
	...
	private static final Set<String> FORMATS = Collections
			.singleton("image/png");

	private static final Set<String> LEGEND_FORMAT = Collections
			.singleton("image/png");

	/** Test layers */
	public static final QName REGIONATED = new QName(MockData.SF_URI,
			"Regionated", MockData.SF_PREFIX);
	public static final QName ACCIDENT = new QName(MockData.SF_URI, "Accident",
			MockData.SF_PREFIX);
	public static final QName ACCIDENT2 = new QName(MockData.SF_URI,
			"Accident2", MockData.SF_PREFIX);
	public static final QName ACCIDENT3 = new QName(MockData.SF_URI,
			"Accident3", MockData.SF_PREFIX);
	...
	/**
	 * Adds required styles to test the selection of maximum and minimum
	 * denominator from style's rules.
	 */
	@Override
	...
	void addLayerAndStyle(SystemTestData testData, QName name)
			throws IOException {
		testData.addVectorLayer(name, null,
				name.getLocalPart() + ".properties", getClass(), this.catalog);
	...
	public void scaleHintDefaultMinValue() throws Exception {
		...
		assertEquals(Double.valueOf(253427.07037725858),
	...
	public void scaleHintDefaultMaxValue() throws Exception {
		...
		assertEquals(Double.valueOf(126713.53518862929),
	...
	public void scaleHintFoundMaxMinDenominators() throws Exception {
		...
		assertEquals(Double.valueOf(31678.383797157323),
				Double.valueOf(scaleElement.getAttribute("min")));
		assertEquals(Double.valueOf(253427.07037725858),
				Double.valueOf(scaleElement.getAttribute("max")));
	}
	...
	private Document findCapabilities() throws Exception {
		...
		GetCapabilitiesTransformer tr = new GetCapabilitiesTransformer(wms,
				BASE_URL, FORMATS, LEGEND_FORMAT, null);
		...
		Assert.assertEquals(WMS.VERSION_1_1_1.toString(),
	 ...
	 */
	private Element searchLayerElement(final String layerRequired,
			Document capabilities) throws XpathException {

		NodeList layersNodes = xpath.getMatchingNodes("//Layer/Name",
				capabilities);
		for (int i = 0; i < layersNodes.getLength(); i++) {
			...
			for (int j = 0; j < childNodes.getLength(); j++) {
				...
				if (layerRequired.equalsIgnoreCase(nodeValue)) {
					...
					return (Element) e.getParentNode(); // returns the layer
